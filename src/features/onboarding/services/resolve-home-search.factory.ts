...
import { flattenRankingGroupsToRankedFunds } from '@/features/funds/services/get-rankings.factory';
import {
...
/**
 * Dependencies for the home-search resolver.
 */
export type HomeSearchServiceDeps = {
  getRankingsGrouped: (options?: {
    limit?: number;
    groupsLimit?: number;
    signal?: AbortSignal;
  }) => Promise<BenchmarkRankingGroup[]>;
  explainAssistant: (request: AssistantExplainRequest) => Promise<AssistantExplainResponse>;
...
/**
 * Creates the home-search resolver without React Native imports.
 *
 * @param deps - Rankings loader, assistant explain port, and fallback predicate.
 */
export function createHomeSearchService(deps: HomeSearchServiceDeps) {
  const { getRankingsGrouped, explainAssistant, allowsMockFallback } = deps;
  ...
  async function resolveHomeSearch(
    query: string,
