...
/**
 * Creates a compare-selection store bound to a key/value persistence port.
 *
 * @param storage - Persistence adapter.
 */
export function createCompareSelectionStore(storage: KeyValueStoragePort) {
  ...
  async function readSelection(): Promise<string[]> {
    await storage.migrateLegacy(COMPARE_SELECTION_STORAGE_KEY);
    const raw = await storage.read(COMPARE_SELECTION_STORAGE_KEY);
  ...
  async function writeSelection(isins: readonly string[]): Promise<void> {
    ...
    await storage.write(COMPARE_SELECTION_STORAGE_KEY, JSON.stringify(unique));
  }

  return {
    subscribe(listener: CompareSelectionListener): () => void {
