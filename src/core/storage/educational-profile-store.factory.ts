...
/**
 * Parses a persisted educational profile payload.
 *
 * @param value - Raw JSON value from storage.
 */
export function parseEducationalProfile(value: unknown): EducationalProfile | null {
  if (!isRecord(value)) {
    return null;
  }
...
/**
 * Creates an educational-profile store bound to a key/value persistence port.
 *
 * @param storage - Persistence adapter.
 */
export function createEducationalProfileStore(storage: KeyValueStoragePort) {
  const listeners = new Set<EducationalProfileListener>();
  /** Last profile written in this store instance; avoids gate races when SecureStore is slow. */
  ...
  return {
    subscribe(listener: EducationalProfileListener): () => void {
    ...
    async getProfile(): Promise<EducationalProfile | null> {
      ...
      try {
        await storage.migrateLegacy(EDUCATIONAL_PROFILE_STORAGE_KEY);
        const raw = await storage.read(EDUCATIONAL_PROFILE_STORAGE_KEY);
    ...
    async saveProfile(profile: EducationalProfile): Promise<void> {
      ...
      try {
        await storage.write(EDUCATIONAL_PROFILE_STORAGE_KEY, JSON.stringify(profile));
      } catch (cause) {
        throw new AppError(
    ...
    async clearProfile(): Promise<void> {
      ...
      try {
        await storage.remove(EDUCATIONAL_PROFILE_STORAGE_KEY);
      } catch (cause) {
        throw new AppError(
          'STORAGE_WRITE_FAILED',
