...
Onboarding policy (platform-specific): native iOS/Android users must complete the profiling questionnaire (`/questionnaire?mode=initial`) on first launch, with an explicit **Omitir** escape in the header; web users land on the dashboard with educational invitations only (no gate). The Aprendizaje tab lives at `/learn` (curriculum). See [docs/product/problem-statement.md](docs/product/problem-statement.md) §5.2.1.
...
- Testing follows clean-architecture layers (pyramid as maturity baremo, not a sprint quota): **domain → unit**, **use cases → integration**, **adapters → contract (~100% of touched public surface)**, **critical journeys → few e2e**. See [docs/architecture/testing-strategy.md](docs/architecture/testing-strategy.md), [ADR-004](docs/architecture/adr-004-testing-by-architecture-layer.md), and `.cursor/rules/testing-strategy.mdc`. Climb toward the baremo incrementally when touching each layer.
