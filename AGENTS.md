...
- CI also runs `build:web:ci` and `verify:prebuild`. On push to `main`, EAS Workflow `.eas/workflows/main-deployments.yml` deploys web only (`alias: inversora`). Android and iOS preview builds stay manual (`pnpm run build:preview:android` / `pnpm run build:preview:ios`, or Expo dashboard workflow **Preview build (Android)**) to avoid EAS Build credit burn on every merge.
